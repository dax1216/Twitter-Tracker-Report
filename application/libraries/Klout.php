 ...
 * @created 08/25/2010
 * Adapted from Phil Sturgeon's REST client (http://philsturgeon.co.uk/code)
 */
class Klout {

    private $CI; // CodeIgniter instance
    private $api_url = 'http://api.klout.com/1/';
    private $api_key;
    ...
    public function get($uri, $params = array(), $format = NULL) {
        if (!empty($params)) {
            $uri .= '.'  . $format . '?' . 'key=' . $this->api_key . '&' . http_build_query($params);
        }
